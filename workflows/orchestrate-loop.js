...
const IMPL_SCHEMA = {
  ...
  properties: {
    ...
    baseSha: { type: 'string', description: 'git merge-base origin/main HEAD — where this branch left main' },
...
function ledgerPrompt(payload) {
  return `${brief('ledger')}- Branch: \`harness/run-<date>-${payload.projectSlug}\` (date = \`date +%F\`)
...
function implPrompt(task, fixFindings, resolved) {
  ...
  git -C ${path} rev-parse --verify --quiet ${task.runBranch} && git -C ${path} worktree add ${WORKTREE_DIR}/${wtName(task)} -b ${task.laneBranch} ${task.runBranch} || git -C ${path} worktree add ${WORKTREE_DIR}/${wtName(task)} -b ${task.laneBranch} origin/main
...
## This dispatch
- Repo: ${path}; branch ${task.branch || '(create the feature branch off origin/main)'}; PR title tag ${ticketTag(task)}.
...
1. \`git -C ${path} rev-parse --verify --quiet ${task.runBranch}\` — if missing: \`git -C ${path} branch ${task.runBranch} origin/main\`.
...
function terminalTask(repo) {
  ...
  return {
    ...
    git -C ${path} diff origin/main...${ref.branch}

...
**The code graph: a map for research, the code is the territory.** `tools/graph` indexes every repository of the project (tree-sitter, 14 languages with relations: TypeScript/JavaScript, Python, Rust, Go, Java, Kotlin, C, C++, C#, Ruby, PHP, Swift) into a local SQLite graph of definitions and their `CALLS`, `IMPORTS`, `EXTENDS` / `IMPLEMENTS` edges, test markers and git co-change, served to agents as the `graph_*` MCP tools. It is for **research**: *where* something is and *what relates to what* (callers, call paths, implementors, the blast radius of a change, the files a slice will touch). It never answers *what the code does* or *whether it is right*: those are read in the code, at the `path:line` the graph points to, before anything is stated, planned, built or reviewed. Its edges are name-resolved leads carrying a confidence; a missing edge proves nothing (dynamic dispatch, reflection, config wiring, other repositories), so "no callers" is confirmed with grep. Who uses it: the scouts (`codebase-scout`, `contract-checker`, `security-scout`, `perf-scout`) and the design skills through them (`roast`, `to-plan`, `to-issues`) as a first pass; implementers only to locate what they then open; the `reviewer` never, since a verdict rests on the diff and the code alone. Reference: `tools/graph/README.md`.
